...
function writeIdentityDocs(targetRoot, projectName) {
  ...
  writeRelative(
    ...
    markdown([
      ...
      "Host and local Git excludes cannot hide active source; any active repository-local Git exclude",
      "rule blocks `pnpm goal:new`.",
      ...
      "trusted project Stop hook refreshes changed sources at turn boundaries; semantic search",
      ...
      "a force-push reason. It also rejects any active repository-local Git exclude rule. The preceding",
      "push owns remote authentication and publication.",
      "",
      ...
      "Treat product identity and public contact/deployment data as configuration with one user-approved machine-readable owner; machine consumers derive from it without literal fallbacks.",
      "Use explicit placeholders or RFC-reserved domains until configured. Keep secrets and machine-local state out of Git, run focused checks, and run `mise exec --locked -- pnpm verify` once.",
    ]),
  );
  writeRelative(
    targetRoot,
    ...
    markdown([
      ...
      "After setup, the project-local Codex Stop hook refreshes changed indexed sources incrementally",
      "before each turn ends; semantic search also repairs freshness on demand. Review and",
      "trust a new or changed hook definition locally with `/hooks` before it can run.",
      "Indexing replaces a complete generation after 20 incremental operations or 100,000 affected",
      "rows, reusing only verified vectors; corruption and older schemas rebuild without reuse.",
      "`context:check`, verification, and pre-push stay read-only; see [Context Index](docs/context-index.md).",
      "",
      ...
      "generated indexes, or optional working context. Project identity and public contact/deployment values get one user-approved machine-readable owner; machine consumers derive from it without literal fallbacks.",
      "`docs/project-context.md`, when present, is only a bounded current-goal cache. Operational examples use explicit placeholders or RFC-reserved domains until configured.",
      "Source, tests, and configuration remain implementation truth; keep normal planning and status in the conversation.",
      "",
      ...
      "another one. It is a read-only fail-closed publication gate, rejects any active repository-local",
      "Git exclude rule, and is not a planning-state generator.",
    ]),
  );
  writeRelative(
    targetRoot,
    ...
    markdown([
      ...
      "    configured remote-tracking upstream, and it rejects any active repository-local Git exclude",
      "    rule; the preceding push owns remote authentication.",
      "",
  ...
  writeRelative(
    ...
    markdown([
      ...
      "  repair. New or changed hook hashes require local `/hooks` approval.",
      "- Indexing atomically replaces complete generations at 20 operations or 100,000 affected rows,",
      "  with verified reuse but no reuse for corruption or old schemas. Ignored index writes do not",
      ...
      "  it also rejects every active repository-local Git exclude rule.",
...
function main() {
  ...
  try {
    ...
    runNode(stagingRoot, "scripts/setup/validate-staged-project.mjs");
